library(keras)
#Load dataset
data <- read.csv("sunspots.csv")
head(data)

data <- data.matrix(data[1:3250, ])
#data <- data[1:2000,]
...
plot(data[1:3250,],type = "l")
n <- nrow(data)
train_data <- data[1:(0.8 * n), ]
train_data <- array_reshape(train_data,dim = c(2600,1))
test_data <- data[(0.8 * n + 1):n, ]
test_data <- array_reshape(test_data,dim = c(650,1))
...
train_sequences <- array(0,dim = c(2597,3,1))
train_targets <- array(0,dim=c(2597,1))
...
test_sequences <- array(0,dim = c(647,3,1))
test_targets <- array(0,dim=c(647,1))
...
test_predictions <- array(0,dim=c(647,1))
test_pid1 <- rep(0,647)
test_pid2 <- rep(0,647)
...
for (i in 1:dim(test_sequences)[1]){
  if(i<=1)test_predictions[i, 1] <- model %>% predict(array_reshape(test_sequences[i,,],dim = c(1,3,1)),verbose = 0)
    ...
    if(pid_output1 > 5) pid_output1 <- 5
    else if (pid_output1 < -5) pid_output1 <- -5
...
plot(test_pid1[2:647],type = "l",col="red")
lines(test_predictions[2:647,1],type = "l",col="black")
lines(test_targets[2:647,1],col="blue")
...
true_value=test_targets[2:647,1];predict_value=test_pid1[2:647]
...
e1<-abs(test_predictions[2:647,1]-test_targets[2:647,1])
e2<-abs(test_pid1[2:647]-test_targets[2:647,1])
