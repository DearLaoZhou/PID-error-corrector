...
data <- data.matrix(data[1:10000, 1:3])
#data <- data[1:2000,]
...
train_data <- array_reshape(train_data,dim = c(8000,3))
test_data <- data[(0.8 * n + 1):n, ]
test_data <- array_reshape(test_data,dim = c(2000,3))

sequence_length <- 3#sliding window

train_sequences <- array(0,dim = c(7997,3,3))
train_targets <- array(0,dim=c(7997,2))


#Construct input-output data
for (i in sequence_length:(dim(train_data)[1]-1)) {
  train_data1 <- train_data[(i - sequence_length + 1):i, 1:3]
  train_data1 <- array_reshape(train_data1,dim=c(1,3,3))
  train_sequences[i - sequence_length + 1, , ] <- train_data1
  train_targets[(i - sequence_length + 1),] <- train_data[(i + 1), 2:3] #
}
...
test_sequences <- array(0,dim = c(1997,3,3))
test_targets <- array(0,dim=c(1997,2))

#Construct test data
for (i in sequence_length:(dim(test_data)[1] - 1)) {
  test_data1 <- test_data[(i - sequence_length + 1):i, 1:3]
  test_data1 <- array_reshape(test_data1,dim=c(1,3,3))
  test_sequences[i - sequence_length + 1, , ] <- test_data1
  test_targets[(i - sequence_length + 1),] <- test_data[(i + 1), 2:3]
}

test_predictions <- array(0,dim=c(1997,2))
...
for (i in 1:dim(test_sequences)[1]){
  if(i<=1)test_predictions[i, ] <- model %>% predict(array_reshape(test_sequences[i,,],dim = c(1,3,3)),verbose = 0)
  else{
    test_predictions[i, ] <- model %>% predict(array_reshape(test_sequences[i,,],dim = c(1,3,3)),verbose = 0)
    #PID error corrector 1
    kp1=1.0;ki1=0.1;kd1=0.01;dt1 <- 0.1
    #dt1 <- 0.1;prev_error1 <- 0;error_sum1<-0
...
plot(test_pid2[2:1997],type = "l",col="red")
lines(test_predictions[2:1997,2],type = "l",col="black")
lines(test_targets[2:1997,2],col="blue")
...
true_value=test_targets[2:1997,2];predict_value=test_predictions[2:1997,2]
...
#z
e11<-abs(test_predictions[2:1997,2]-test_targets[2:1997,2])
e22<-abs(test_pid2[2:1997]-test_targets[2:1997,2])
plot(e11,type = "l",col="red") #LSTM
lines(e22,type = "l",col="black")#LSTM-PID
